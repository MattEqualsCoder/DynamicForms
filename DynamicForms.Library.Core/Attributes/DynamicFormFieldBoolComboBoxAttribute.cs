namespace DynamicForms.Library.Core.Attributes;

/// <summary>
/// Attribute for creating a combo box for bool values that equal to either "yes" or "no"
/// </summary>
/// <param name="label">The form label text</param>
/// <param name="labelIsProperty">If the label is a property instead of text</param>
/// <param name="toolTipText">Text to display when hovering over the object</param>
/// <param name="visibleWhenTrue">Property to look at to determine if the field should be shown or not</param>
/// <param name="editableWhenTrue">Property to look at to determine if the field should be editable or not</param>
/// <param name="groupName">The group the field should be on</param>
/// <param name="order">The order to show the field in</param>
/// <param name="platforms">The platform(s) the object should be displayed on</param>
[AttributeUsage((AttributeTargets.Property))]
public class DynamicFormFieldBoolComboBoxAttribute(
    string label = "",
    bool labelIsProperty = false,
    string trueText = "Yes",
    string falseText = "No",
    bool trueFirst = true,
    bool allowNull = false,
    string nullText = "",
    string? toolTipText = null,
    string? visibleWhenTrue = null,
    string? editableWhenTrue = null,
    string groupName = "",
    int order = int.MaxValue,
    DynamicFormPlatform platforms = DynamicFormPlatform.All)
    : DynamicFormFieldAttribute(label, labelIsProperty, toolTipText, visibleWhenTrue, editableWhenTrue, groupName, order, platforms)
{
    public override DynamicFormFieldType FieldType => DynamicFormFieldType.BoolComboBox;
    public override ICollection<Type> AllowedTypes => [typeof(bool)];
    
    public IReadOnlyCollection<string> GetComboBoxOptions()
    {
        if (allowNull)
        {
            return trueFirst ? [nullText, trueText, falseText] : [nullText, falseText, trueText];
        }

        return trueFirst ? [trueText, falseText] : [falseText, trueText];
    }

    public string GetSelectedOption(object? value)
    {
        if (value is bool nullableValue)
        {
            return nullableValue ? trueText : falseText;
        }

        if (allowNull)
        {
            return nullText;
        }
        
        return trueFirst ? trueText : falseText;
    }

    public bool? GetSelectedValue(string? option)
    {
        if (option == trueText)
        {
            return true;
        }
        else if (option == falseText)
        {
            return false;
        }

        return allowNull ? null : trueFirst;
    }
}